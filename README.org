...
categorize, subtitle and serve your favorite shows, movies, audio and books as well a used a document 
storage on network (MinIO).
...
- *An installation location*: The script is going to ask you for a install location, but it will default
  to ~/opt/jams~. Just make sure your current user has permissions to write on the selected directory.
...
Usage: jans [--help|restart|stop|start]
options:
